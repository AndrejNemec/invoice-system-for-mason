...
function replaceUnitPattern($pattern, $arrayOfReplacements)
{
    foreach ($arrayOfReplacements as $key => $value) {
        $pattern = str_replace($key, $value, $pattern);
    }
    return $pattern;
}

function checkItem($projectItems, $catalogItem)
{
    foreach ($projectItems as $projectItem) {
        if ($catalogItem->id == $projectItem->item->id) return true;
    }
    return false;
}
